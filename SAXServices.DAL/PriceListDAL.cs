...
{
    public class PriceListDAL : CRUDDALBase, ICRUDDAL<PriceList>
    {
        private const string NO_VARIANTES = ".";

        public bool Delete(PriceList element)
        {
        ...
        public IEnumerable<PriceList> Get()
        {
            ...
            {
                result.AddRange(GetPriceList(connection, DateTime.Now));
            }

            return result;
        }

        private IEnumerable<PriceList> GetPriceList(ConnectionStringSettings connection, DateTime fechaVigencia)
        {
            ...
            if (OpenDBConnection(connection.ConnectionString))
            {
                ...
                var sSql = String.Format(CultureInfo.CurrentCulture,
                    ...
                    "WHERE Convert(date, pp.FVigencia) <= '{0}' and Convert(date, pp.FHasta) >= '{0}' and st.Web = 1" +
                    "UNION " +
                    ...
                    "WHERE st.Web = 1  and Convert(date, pph.FVigencia) <= '{0}' and Convert(date, pph.FHasta) >= '{0}'", fechaVigencia.ToString("yyyyMMdd", CultureInfo.CurrentCulture));
                ...
                using (var sqlCommand = new SqlCommand(sSql, oConexion))
                {
                    ...
                    while (rsp.Read())
                    {
                        ...
                        if (!listaPrecios.ContainsKey((int)rsp["id_lista_precios"])){                            
                            listaPrecios.Add((int)rsp["id_lista_precios"], new PriceList{Name = connection.Name + rsp["Descripcion"].ToString(),
                ...
                foreach (var lista in listaPrecios){
                    ...
                    result.Add(
                                new PriceList {Name = lista.Value.Name,
        ...
        public PriceList GetByName(string name)
        {
            ...
            foreach (ConnectionStringSettings connection in connections)
            {
                if (name.Substring(0, connection.Name.Length) == connection.Name)
                    return GetPriceList(connection, DateTime.Now).FirstOrDefault(l => l.Name == name);
            }
