...
{
    public class OrderDAL : CRUDDALBase, ICRUDDAL<Order>
    {
        private const string NO_VARIANTES = ".";

        public bool Delete(Order element)
        {
        ...
        private void GetOrderData(ConnectionStringSettings connection, int id, ref List<Order> result)
        {
            if (OpenDBConnection(connection.ConnectionString))
            {
                var orderDetailList = new Dictionary<int, List<OrderDetail>>();
                ...
                using (var sqlCommand = new SqlCommand(sSql, oConexion))
                {
                    ...
                    while (rsp.Read())
                    {
                        ...
                        {
                            orderDetailList.Add((int)rsp["ID"], new List<OrderDetail>());
                        }
//                      aqui
//                      var product_id = String.Concat(rsp["Producto_ID"].ToString().TakeWhile(c => Char.IsNumber(c)));
        ...
        public bool Save(Order element, out String mensaje)
        {
            var connections = ConfigurationManager.ConnectionStrings;
            var result = true;
            mensaje = "0";
        ...
        private bool SaveOrder(ConnectionStringSettings connection, Order order, out String mensaje)
        {
            mensaje = "0";
            if (OpenDBConnection(connection.ConnectionString))
            {
 ...
 //                  "FROM Orden_De_Compra_Cliente";
                var sSql = "SELECT IsNull(MAX(Num), 0) + 1 as Proximo_Numero FROM(SELECT(numero_orden) AS Num FROM orden_de_compra_cliente UNION SELECT(numero_orden) AS Num FROM orden_de_compra_cliente_h) u";
